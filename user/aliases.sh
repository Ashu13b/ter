...
alias about-device='am start -a android.settings.DEVICE_INFO_SETTINGS >/dev/null 2>&1 || adb shell am start -a android.settings.DEVICE_INFO_SETTINGS >/dev/null 2>&1 || termux-notification -t "About Device" -c "Tap to open settings" --action "am start -a android.settings.DEVICE_INFO_SETTINGS"'
