...
_ter_apply_theme() {
    ...
    # ~/.tmux.conf is a symlink to the per-user runtime copy
    # (~/.config/ter/tmux.conf, seeded by install.sh). Resolve the symlink so
    # theme edits land on the real file and the repo stays clean.
    [ ! -e "$conf_file" ] && return 0
    local targets
    targets=$(readlink -f "$conf_file" 2>/dev/null || echo "$conf_file")
...
# key -> "inactive|active|accent|p_line|p_accent|Name"
# (first three drive the tmux status bar; p_* drive the native prompt via
#  ~/.config/ter/prompt_colors.sh)
_ter_theme_spec() {
    case "$1" in
        C|c|solarized)  printf '%s\n' "108|253|136|108|136|Solarized & Sage Green" ;;
        F|f|midnight)   printf '%s\n' "189|253|211|146|176|Midnight Indigo & Soft Lavender" ;;
        G|g|charcoal)   printf '%s\n' "223|187|215|180|209|Charcoal Coffee & Warm Sand" ;;
        H|h|aubergine)  printf '%s\n' "224|181|173|174|168|Aubergine Wine & Peach Cream" ;;
        I|i|obsidian)   printf '%s\n' "179|137|179|179|214|Obsidian Black & Amber Gold" ;;
        J|j|nord)       printf '%s\n' "110|139|75|110|75|Nord Frost & Glacier Blue" ;;
        K|k|ocean)      printf '%s\n' "66|109|80|73|80|Ocean Deep & Aqua Glow" ;;
        L|l|rose)       printf '%s\n' "181|218|168|181|218|Rose Quartz & Blush Pink" ;;
        M|m|matrix)     printf '%s\n' "65|114|46|71|118|Matrix Emerald & Neon Lime" ;;
        N|n|sunset)     printf '%s\n' "131|209|202|173|202|Sunset Ember & Coral Bloom" ;;
    esac
}
...
# Write per-user prompt colors; sourced by core/theme.sh on every startup.
# Lives outside the repo so theme choice survives reinstalls cleanly.
_ter_apply_prompt_colors() {
    local dir="$HOME/.config/ter"
    mkdir -p "$dir"
    cat > "$dir/prompt_colors.sh" <<EOF
# Generated by 'ter theme' — per-user native-prompt colors (256-color).
export TER_P_LINE='$1'
export TER_P_ACCENT='$2'
EOF
}

_ter_set_theme() {
    local spec inactive active rest accent pline paccent name
    ...
    _ter_apply_prompt_colors "$pline" "$paccent"
